...
namespace KMCCC.Modules.Yggdrasil
{
	...
	public class YggdrasilClient
	{
		...
		private void UpdateFomrResponse(AuthenticationResponse response)
		{
            ...
            GBCLV2.Config.UserName = DisplayName;
		...
		public string Authenticate(string email, string password, string ExToken = null, bool twitchEnabled = true)
		{
			lock (_locker)
			{
				Clear();
                ...
                try
                {
                    responseBody = wc.UploadString(new Uri(Auth_Authentication), requestBody);
                }
                catch (WebException e)
                {
                    if (e.Response == null) return "请检查网络连接";
                    return e.Message + "\n请检查邮箱与密码是否正确";
                }
				var response = JsonMapper.ToObject<AuthenticationResponse>(responseBody);
				if (response.AccessToken == null)
